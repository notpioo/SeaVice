...
export default function AdminPulsaProducts() {
  const { toast } = useToast();
  const [selectedBrand, setSelectedBrand] = useState("all");
  const [searchQuery, setSearchQuery] = useState("");
  const [editingProduct, setEditingProduct] = useState<PulsaService | null>(null);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isGlobalDialogOpen, setIsGlobalDialogOpen] = useState(false);

  // Fetch raw pulsa services with customization info
  const { data: pulsaData, isLoading } = useQuery({
    queryKey: ["/api/pulsa/services/raw", "pulsa-reguler", selectedBrand],
    queryFn: async () => {
  ...
  const globalPulsaMarkup = useMemo(() => {
    return globalMarkupData?.data?.find((m: GlobalMarkup) => m.productType === "pulsa") || {
      productType: "pulsa",
      markupType: "fixed",
      markupValue: 500,
      isActive: true,
  ...
  // Form for product customization
  ...
  // Reset global form when data loads
  useEffect(() => {
    if (globalPulsaMarkup) {
      globalForm.reset(globalPulsaMarkup);
    }
  }, [globalPulsaMarkup, globalForm]);
  ...
  const toggleVisibilityMutation = useMutation({
    mutationFn: async ({ product, isVisible }: { product: PulsaService; isVisible: boolean }) => {
  ...
  const handleEditProduct = (product: PulsaService) => {
  ...
  const filteredServices = useMemo(() => {
    ...
    return pulsaData.data.filter((service: PulsaService) => {
  ...
  }, [pulsaData, searchQuery]);

  // Calculate selling price for display
  const calculateSellingPrice = (service: PulsaService) => {
    ...
    if (globalPulsaMarkup?.isActive && globalPulsaMarkup.markupValue > 0) {
      if (globalPulsaMarkup.markupType === "percentage") {
        return Math.ceil(basePrice * (1 + globalPulsaMarkup.markupValue / 100));
      }
      return basePrice + globalPulsaMarkup.markupValue;
    }
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold">Kelola Produk Pulsa</h1>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-lg bg-primary/10">
                {globalPulsaMarkup.markupType === "percentage" ? (
              ...
              <div>
                <p className="font-medium">Markup Global Pulsa</p>
                <p className="text-sm text-muted-foreground">
                  {globalPulsaMarkup.isActive ? (
                    globalPulsaMarkup.markupType === "percentage" 
                      ? `+${globalPulsaMarkup.markupValue}% dari harga modal`
                      : `+${formatPrice(globalPulsaMarkup.markupValue)} per produk`
                  ) : (
                    "Tidak aktif"
                  )}
                </p>
            ...
            <Badge variant={globalPulsaMarkup.isActive ? "default" : "secondary"}>
              {globalPulsaMarkup.isActive ? "Aktif" : "Nonaktif"}
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3">
          {filteredServices.map((service: PulsaService) => {
      ...
      <Dialog open={isGlobalDialogOpen} onOpenChange={setIsGlobalDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Markup Global Pulsa</DialogTitle>
            <DialogDescription>
              Atur markup default untuk semua produk pulsa
            </DialogDescription>
          </DialogHeader>
          <Form {...globalForm}>
