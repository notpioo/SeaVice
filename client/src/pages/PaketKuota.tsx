...
import { Smartphone, Search, ArrowLeft, Wifi } from "lucide-react";
...
export default function PaketKuota() {
  ...
  const { data: kuotaData, isLoading, error } = useQuery<KuotaResponse>({
    queryKey: ["/api/pulsa/services", "paket-internet", detectedCarrier?.brand],
    queryFn: async () => {
  ...
  const categories = useMemo(() => {
    ...
    const cats = new Set(kuotaData.data.map((s) => s.category));
  ...
  const filteredServices = useMemo(() => {
    ...
    return kuotaData.data.filter((service) => {
  ...
  }, [kuotaData, selectedCategory, searchQuery]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30">
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">Paket Kuota Internet</h1>
            <p className="text-muted-foreground text-sm">Beli paket data internet dengan mudah</p>
          </div>
        </div>
        ...
        {detectedCarrier && (
          <>
            ...
            {!isLoading && !error && kuotaData && (
              <div className="space-y-6">
                {sortedServices.length > 0 && (
                  <div>
                    <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
                      <Wifi className="h-5 w-5" />
                      Paket Internet {detectedCarrier?.displayName}
                    </h2>
                    ...
                    <CardContent className="p-8 text-center">
                      ...
                      <p className="text-muted-foreground">
                        {searchQuery || selectedCategory
                          ...
                          : `Tidak ada paket kuota tersedia untuk ${detectedCarrier?.displayName || "operator ini"}`}
                      </p>
                    </CardContent>
                  </Card>
                )}
        ...
        {!detectedCarrier && phoneNumber.length === 0 && (
          <Card>
            <CardContent className="p-8 text-center">
              <Wifi className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
              <p className="text-muted-foreground">
                Masukkan nomor HP untuk melihat paket kuota yang tersedia
              </p>
            </CardContent>
          </Card>
        )}
